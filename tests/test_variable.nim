...
suite "Variable creation":
  test "create simple variable":
    let v = initVariable("Age", "A", Cardinality(3))
    check v.name == "Age"
    check v.abbrev == "A"
  ...
  test "create dependent variable":
    let v = initVariable("Output", "Z", Cardinality(2), isDependent = true)
    check v.isDependent == true

  test "abbreviation is capitalized":
    let v = initVariable("test", "abc", Cardinality(2))
    check v.abbrev == "Abc"
  ...
  test "bitSize calculated correctly":
    ...
    let v2 = initVariable("Binary", "B", Cardinality(2))
    check v2.bitSize == 2

    # cardinality 3 needs 2 bits (values 0,1,2 + DontCare=3)
    let v3 = initVariable("Ternary", "T", Cardinality(3))
    check v3.bitSize == 2

    # cardinality 4 needs 3 bits (values 0-3 + DontCare=7)
    let v4 = initVariable("Quad", "Q", Cardinality(4))
    check v4.bitSize == 3

    # cardinality 5 needs 3 bits (values 0-4 + DontCare=7)
    let v5 = initVariable("Five", "F", Cardinality(5))
    check v5.bitSize == 3

    # cardinality 256 needs 9 bits (values 0-255 + DontCare=511)
    let v256 = initVariable("Large", "L", Cardinality(256))
...
suite "VariableList basic operations":
  ...
  test "add single variable":
    ...
    let idx = list.add(initVariable("Age", "A", Cardinality(3)))
    check list.len == 1
    check idx == VariableIndex(0)

  test "add multiple variables":
    var list = initVariableList()
    discard list.add(initVariable("Var1", "V1", Cardinality(3)))
    discard list.add(initVariable("Var2", "V2", Cardinality(4)))
    discard list.add(initVariable("Var3", "V3", Cardinality(2)))
    check list.len == 3

  test "access by index":
    var list = initVariableList()
    discard list.add(initVariable("Age", "A", Cardinality(3)))
    discard list.add(initVariable("Income", "I", Cardinality(5)))
  ...
  test "abbreviation lookup is case insensitive":
    var list = initVariableList()
    discard list.add(initVariable("Age", "Age", Cardinality(3)))
...
suite "VariableList bit-packing":
  test "first variable starts at top of first segment":
    ...
    discard list.add(initVariable("Binary", "B", Cardinality(2)))
  ...
  test "second variable follows first in same segment":
    ...
    discard list.add(initVariable("First", "F", Cardinality(3)))
    # 3 bits for cardinality 4 (needs space for DontCare)
    discard list.add(initVariable("Second", "S", Cardinality(4)))
  ...
  test "new segment when current is full":
    ...
    for i in 0..<10:
      discard list.add(initVariable("Var" & $i, "V" & $i, Cardinality(4)))

    # Next 3-bit variable won't fit (30 + 3 = 33 > 32), goes to segment 1
    let idx = list.add(initVariable("Overflow", "O", Cardinality(4)))
    let v = list[idx]
  ...
  test "keySize reflects number of segments needed":
    ...
    for i in 0..<16:
      discard list.add(initVariable("V" & $i, "V" & $i, Cardinality(3)))
    check list.keySize == 1  # Exactly 32 bits

    # Now overflow to second segment
    discard list.add(initVariable("Over", "O", Cardinality(2)))
...
suite "VariableList directed/neutral":
  test "isDirected false when no dependent variables":
    var list = initVariableList()
    discard list.add(initVariable("A", "A", Cardinality(2)))
    discard list.add(initVariable("B", "B", Cardinality(2)))
    check list.isDirected == false

  test "isDirected true when has dependent variable":
    var list = initVariableList()
    discard list.add(initVariable("A", "A", Cardinality(2)))
    discard list.add(initVariable("Z", "Z", Cardinality(2), isDependent = true))
...
suite "VariableList iteration":
  test "iterate over items":
    var list = initVariableList()
    discard list.add(initVariable("A", "A", Cardinality(2)))
    discard list.add(initVariable("B", "B", Cardinality(3)))
    discard list.add(initVariable("C", "C", Cardinality(4)))
...
suite "Variable edge cases":
  test "should handle cardinality of 1":
    let v = initVariable("Single", "S", Cardinality(1))
  ...
  test "should handle very large cardinality":
    let v = initVariable("VeryLarge", "VL", Cardinality(10000))
  ...
  test "should capitalize single-char abbreviation":
    let v = initVariable("test", "a", Cardinality(2))
    check v.abbrev == "A"
  ...
  test "should handle empty name":
    let v = initVariable("", "X", Cardinality(2))
    check v.name == ""
    check v.abbrev == "X"
...
suite "VariableList edge cases":
  ...
  test "should handle multiple dependent variables":
    var list = initVariableList()
    discard list.add(initVariable("A", "A", Cardinality(2)))
    discard list.add(initVariable("Z1", "Y", Cardinality(2), isDependent = true))
    discard list.add(initVariable("Z2", "Z", Cardinality(2), isDependent = true))
  ...
  test "should handle all variables being dependent":
    var list = initVariableList()
    discard list.add(initVariable("Z1", "A", Cardinality(2), isDependent = true))
    discard list.add(initVariable("Z2", "B", Cardinality(2), isDependent = true))

    check list.isDirected == true

  test "should handle building mask for empty variable indices":
    var list = initVariableList()
    discard list.add(initVariable("A", "A", Cardinality(4)))
    discard list.add(initVariable("B", "B", Cardinality(4)))
  ...
  test "should handle many variables filling multiple segments":
    ...
    for i in 0..<32:
      discard list.add(initVariable("V" & $i, $chr(ord('A') + (i mod 26)) & $i, Cardinality(2)))
