...
suite "Key value operations":
  setup:
    ...
    discard varList.add(newVariable("VarA", "A", Cardinality(4)))
    # B: cardinality 3 (2 bits), values 0-2
    discard varList.add(newVariable("VarB", "B", Cardinality(3)))
    # C: cardinality 2 (1 bit), values 0-1
    discard varList.add(newVariable("VarC", "C", Cardinality(2)))
...
suite "Key with multiple segments":
  setup:
    ...
    discard varList.add(newVariable("Overflow", "O", Cardinality(4)))
...
suite "Key matching":
  ...
  test "DontCare matches any value":
    var k1 = newKey(varList.keySize)  # All DontCare
    var k2 = varList.buildKey(@[(VariableIndex(0), 2), (VariableIndex(1), 3)])
    check k1.matchesWithVarList(k2, varList)
...
suite "Key value encoding (behavioral)":
  ...
  setup:
    var varList = initVariableList()
    discard varList.add(newVariable("A", "A", Cardinality(8)))   # 3 bits
    discard varList.add(newVariable("B", "B", Cardinality(16)))  # 4 bits
    discard varList.add(newVariable("C", "C", Cardinality(4)))   # 2 bits
...
suite "Key boundary conditions":
  ...
  test "should handle maximum cardinality variable":
    ...
    discard varList.add(newVariable("Big", "B", Cardinality(65535)))
  ...
  test "should handle many small variables packed together":
    ...
    for i in 0..<32:
      discard varList.add(newVariable("V" & $i, $chr(ord('a') + (i mod 26)), Cardinality(2)))
  ...
  test "should match keys with all DontCare":
    ...
    let k1 = newKey(varList.keySize)  # All DontCare
    let k2 = newKey(varList.keySize)  # All DontCare
...
suite "Bitwise matches function (no varList)":
  ...
  test "all DontCare matches anything":
    var kDontCare = newKey(1)  # All 1s (DontCare)
    var kSpecific = newKey(1)
...
suite "Matching semantics verification":
  ...
  setup:
    var varList = initVariableList()
    discard varList.add(newVariable("A", "A", Cardinality(4)))  # 2 bits
    discard varList.add(newVariable("B", "B", Cardinality(4)))  # 2 bits
